...
class VideoFromFile(VideoInput):
    ...
    def _get_display_dimensions(self) -> tuple[int, int]:
        """Rotation-corrected dimensions of the source, ignoring any crop.

        The crop rect lives in display space, so rotation is resolved from a
        decoded frame like the component and save paths do.
        """
        if isinstance(self.__file, io.BytesIO):
            self.__file.seek(0)
        with av.open(self.__file, mode='r') as container:
